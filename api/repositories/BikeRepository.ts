...
interface IBikeRepository {
  ...
  update(id: number, bikeData: Partial<IUpdateBikeRequest>): Promise<IBike | null>;
}

export class BikeRepository extends BaseRepository<IBike> implements IBikeRepository {
  ...
  async create(bikeData: ICreateBikeRequest): Promise<IBike> {
    try {
      const result = await this.db.runAsync("INSERT INTO bikes (name, brand, created_at, updated_at) VALUES (?, ?, datetime('now'), datetime('now'))", [bikeData.name, bikeData.brand]);
  ...
  async update(id: number, bikeData: Partial<IUpdateBikeRequest>): Promise<IBike | null> {
    try {
      ...
      await this.db.runAsync("UPDATE bikes SET name = ?, brand = ?, updated_at = datetime('now') WHERE id = ?", [updatedName, updatedBrand, id]);
